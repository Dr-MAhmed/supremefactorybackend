    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Record returns of goods to suppliers</p>
        </div>
        {canEdit ? (
          <button
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        {loading ? (
          ...
          <p className="p-6 text-center text-slate-500">No purchase returns yet</p>
        ) : (
          <table className="w-full">
            <thead className="border-b border-slate-200 bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase text-slate-600">Ref. Purchase</th>
            ...
            <tbody>
              {returns.map((r) => (
                <tr key={r.id} className="border-b border-slate-200 hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{r.purchase?.voucherNo || '-'}</td>
                  {canEdit && (
